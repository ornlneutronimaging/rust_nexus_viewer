//! The last few NeXus files opened, persisted across restarts.
//!
//! The list lives in `~/.config/venus_rust_tools/nexus_viewer_recent`, one
//! absolute path per line, most recent first. Persistence is best effort,
//! like the theme preference: an unwritable home directory only loses the
//! list, it never errors.
...
fn list_path() -> Option<PathBuf> {
    ...
    Some(base.join("venus_rust_tools").join("nexus_viewer_recent"))
}
